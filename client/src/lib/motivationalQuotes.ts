export const motivationalQuotes = [
  "Done is better than perfect. Just press record.",
  "You don't need confidence to begin. You gain confidence by beginning.",
  "You're not awkward—you're brave enough to show up.",
  "The cringe fades. The courage stays.",
  "Every video you record builds the speaker within you.",
...
export function getDailyQuote(): string {
  ...
  const today = new Date().toDateString(); // e.g. "Wed Apr 10 2025"
  ...
  if (stored) {
    try {
      ...
      // Continue with new quote if parsing fails
    }
  }

  // Get quote based on day number to keep it consistent
  const index = new Date().getDate() % motivationalQuotes.length;
  const newQuote = motivationalQuotes[index];
  ...
  try {
    ...
    // Continue without storing if localStorage is unavailable
  }
